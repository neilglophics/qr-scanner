...
let cachedDefaultPrinter: Printer | null = null;
...
app.whenReady().then(() => {
  ...
  */
  ipcMain.handle('print', async (_event: Electron.IpcMainInvokeEvent, data: QR, printerName: string, printOption: PrintOption, manualLookup: boolean = false) => {
    ...
    console.log('hereee', defPrinterRes)
    return defPrinterRes;
    // if (!cachedDefaultPrinter) {
    //   cachedDefaultPrinter = await getDefaultPrinter();
    // }
    ...
    // try {
    //   // Print the PDF
    //   await print(localFilePath, { printer: cachedDefaultPrinter.name });
    //   return {
    //     status: 'SUCCESS',
